...
def initialize_shopify_client():
    ...
    if 'api_key' in Context.config:
        ...
        LOGGER.info("Using private app basic authentication from config[\"private_app_api_key\"] and config[\"private_app_password\"]")
        ...
        raise Exception("Config is missing Shopify auth information. Either pass the `api_key` option for a public app authenticated using an access_token via the Shopify OAuth flow, or pass the `private_app_api_key` and `private_app_password` config options for a private app.")
